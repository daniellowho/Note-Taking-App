...
import { IMAGE_OFFICE_REF, PROFILE_ME } from "../data";
...
export default function NoteEditorView({
  ...
  return (
    <div className="w-full min-h-screen bg-background font-sans relative overflow-x-hidden">
      ...
      {/* Formatting Toolbar */}
      <nav className="sticky top-0 bg-background/80 backdrop-blur-md border-y border-outline-variant/20 py-2 px-6">
      ...
      <main className="pb-36 pt-6 overflow-y-auto w-full flex flex-col items-center">
        <div className="w-full max-w-[800px] px-6">
          ...
          <div className="flex flex-wrap items-center gap-2 mb-6 font-sans border-b border-outline-variant/10 pb-4 select-none">
          ...
          {/* Embedded Image matching Screen 3 office reference space */}
          <div className="relative w-full aspect-video rounded-3xl overflow-hidden border border-outline-variant/30 my-8 shadow-sm group">
            <img
              className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105"
              alt="Reference office mockup"
              src={IMAGE_OFFICE_REF}
            />
            <div className="absolute inset-0 bg-gradient-to-t from-black/20 to-transparent"></div>
          </div>

          <p className="text-sm md:text-base text-on-surface-variant leading-relaxed">
            The design system must emphasize tactile softness. We want users to feel empowered and in control, rather than rushed by urgent notifications or high-contrast UI elements.
          </p>

        </div>
      </main>

      {/* Floating Nova Intelligence Button Center bottom */}
      <div className="fixed bottom-8 left-1/2 -translate-x-1/2 z-40">
        <button
          onClick={() => setShowSheet(true)}
          className="bg-primary hover:bg-primary/95 text-on-primary flex items-center gap-3 px-6 py-4 rounded-full shadow-lg hover:shadow-xl hover:scale-105 active:scale-95 transition-all group cursor-pointer"
          id="editor-ai-trigger"
        >
          <Sparkles size={18} className="fill-current animate-pulse text-white" />
          <span className="text-sm font-semibold tracking-wide">Nova Intelligence</span>
        </button>
      </div>
        ...
        <div className="max-w-[800px] mx-auto px-6 py-8">
          ...
          <div className="flex items-center justify-between mb-8 border-b border-outline-variant/10 pb-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-lg md:text-xl font-bold text-on-surface">Nova Intelligence</h3>
                <p className="text-xs text-secondary font-medium">Enhancing your product strategy note</p>
              </div>
            </div>
            <button
              onClick={() => setShowSheet(false)}
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            ...
            <button
              ...
              className="flex flex-col items-start gap-4 p-5 rounded-3xl bg-surface-container-low border border-outline-variant/25 hover:border-primary/30 hover:bg-surface-container transition-all text-left active:scale-[0.98] group cursor-pointer"
            >
              <div className="flex items-center justify-between w-full text-primary">
                ...
                <span className="text-outline-variant text-[11px] font-bold">INFO</span>
              </div>
              <div>
                <span className="font-bold text-sm text-on-surface block mb-1">Summarize</span>
                <span className="text-xs text-secondary leading-snug">Get the key takeaways instantly.</span>
              </div>
            </button>

            {/* Set Reminder */}
            <button
              onClick={handleSetReminderAction}
              className="flex flex-col items-start gap-4 p-5 rounded-3xl bg-surface-container-low border border-outline-variant/25 hover:border-primary/30 hover:bg-surface-container transition-all text-left active:scale-[0.98] group cursor-pointer"
            >
              <div className="flex items-center justify-between w-full text-primary">
                <Clock size={22} className="group-hover:scale-110 transition-transform" />
                <span className="text-outline-variant text-[11px] font-bold">TASK</span>
              </div>
              <div>
                <span className="font-bold text-sm text-on-surface block mb-1">Set Reminder</span>
                <span className="text-xs text-secondary leading-snug">Never miss a follow-up task.</span>
              </div>
            </button>
            ...
            >
              <div className="flex items-center justify-between w-full text-primary">
                ...
                <span className="text-outline-variant text-[11px] font-bold">REPLACE</span>
              </div>
              <div>
                <span className="font-bold text-sm text-on-surface block mb-1">Improve Writing</span>
                <span className="text-xs text-secondary leading-snug">Refine clarity and professional tone.</span>
              </div>
            </button>

          </div>

          {/* Ask custom prompt form */}
          <form onSubmit={handleCustomSubmit} className="bg-surface-container-low/40 border border-outline-variant/20 p-4 rounded-2xl flex items-center gap-3">
            <input
              type="text"
              value={customQuestion}
              onChange={(e) => setCustomQuestion(e.target.value)}
              placeholder="Ask Nova: 'What are the next steps for this project?'"
              className="bg-transparent border-none outline-none focus:outline-none focus:ring-0 text-sm placeholder-on-surface-variant/40 flex-1 h-10 select-text text-on-surface"
              disabled={sheetLoading}
            />
            <button
              type="submit"
              className="p-2.5 rounded-full bg-primary text-white hover:bg-primary/95 transition-all outline-none focus:outline-none cursor-pointer"
              disabled={sheetLoading || !customQuestion.trim()}
          ...
          {/* Footer Suggestion Cues matching Screen 3 */}
          <div className="mt-8 pt-6 border-t border-outline-variant/20 flex items-center justify-center text-center">
            <p className="text-xs text-outline font-medium flex items-center justify-center gap-2 select-none">
              <Lightbulb size={14} className="text-primary" />
              Try clicking:
              <span
                onClick={() => handleQuestionSuggest("What are the next steps for this project?")}
                className="text-primary hover:underline cursor-pointer font-semibold ml-1"
              >
