...
interface VoiceRecorderViewProps {
  ...
  onSaveTranscriptAsNote: (title: string, text: string, duration?: string, audioUrl?: string) => Promise<Note>;
...
export default function VoiceRecorderView({
  ...
  // Render a waveform from the real microphone signal.
  useEffect(() => {
    const barsCount = 24;
    ...
    const setupRealAudio = async () => {
      try {
        ...
        const renderRealWave = () => {
          ...
          // RMS measures actual microphone loudness. Frequency bins then shape each bar.
          let sumSquares = 0;
          ...
          const targetVolume = Math.min(1, rms * 11);
          smoothedVolumeRef.current += (targetVolume - smoothedVolumeRef.current) * 0.22;
          const volume = smoothedVolumeRef.current;

          setWaveformHeights(() =>
            Array.from({ length: barsCount }, (_, idx) => {
              const spectrumIndex = Math.min(bufferLength - 1, Math.floor((idx / barsCount) * bufferLength));
              const spectrumShape = frequencyData[spectrumIndex] / 255;
              const contour = 0.45 + Math.sin(((idx + 1) / (barsCount + 1)) * Math.PI) * 0.55;
              const height = 9 + volume * (18 + contour * 38 + spectrumShape * 35);
              return Math.max(8, Math.min(96, height));
            })
          );
  ...
  const handleEndSession = async () => {
    ...
    // Prefer server transcription of the actual recorded audio. Browser speech
    ...
    // Save only speech recognized from the user's microphone, plus the captured audio.
    const savedNote = await onSaveTranscriptAsNote(generatedTitle, finalContent, durationLabel, audioUrl);

    // Set mock saved ID so we stay in page but can display the processed notification
    setRecentSavedId(mockId);
    // Voice captures are regular notes; open the finished note instead of stranding the user in an archive.
    onSelectNote?.(savedNote);
  };
                        ...
                        <div className="flex items-start justify-between gap-4 pl-1">
                          ...
                          <div className="flex items-center gap-1.5 shrink-0">
                            {onDeleteNote && (
                              <button
                                onClick={() => {
                                  if (playingNoteId === rec.id) handleStopPlayback();
                                  onDeleteNote(rec.id);
                                }}
